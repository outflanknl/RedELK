...
#
# Part of RedELK
# Script to install RedELK on Mythic C2 servers
#
...
if [ ! -f  /etc/apt/sources.list.d/elastic-6.x.list ]; then
    echo "deb https://artifacts.elastic.co/packages/7.x/apt stable main" | tee -a /etc/apt/sources.list.d/elastic-6.x.list >> $LOGFILE 2>&1
fi
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
cp ./filebeat/filebeat_mythic.yml /etc/filebeat/filebeat.yml >> $LOGFILE 2>&1
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
echo ""
echo ""
echo "WARNING - you need to manually adjust the log file of Mythic C2"
echo "You should:"
echo " - set Mythic's 'siem_log_name' config parameter to the corect filename"
echo "   The RedELK installer assumes the value is '/root/Mythic/mythic-docker/redelk.log'"
echo " - adjust 'paths' setting in /etc/filebeat/filebeat.yml to point to this file if you choose a different file path"
echo ""
echo ""