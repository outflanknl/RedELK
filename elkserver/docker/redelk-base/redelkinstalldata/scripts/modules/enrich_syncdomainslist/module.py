...
class Module():
    ...
    def sync_domainslist(self, domainlist='redteam'):
        ...
        for doc in es_domainslist_docs:
            domain = get_value('_source.domainslist.domain', doc)  # pylint: disable=invalid-name
            if domain:
    ...
    def get_cfg_domains(self, domainslist):
        ...
        if not os.path.isfile(fname):
            self.logger.warning('File %s doesn\'t exist, skipping domain list sync for this one.', fname)
            return None
