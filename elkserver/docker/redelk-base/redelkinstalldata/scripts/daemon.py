...
from modules.helpers import (addAlarmData, groupHits, moduleDidRun, setTags,
                             shouldModuleRun)
...
def run_enrichments(enrich_dict):
    ...
    for enrich_module in enrich_dict:
        if shouldModuleRun(enrich_module, 'redelk_enrich'):
            try:
                ...
                for hit in enrich_dict[enrich_module]['result']['hits']['hits']:
                    setTags(enrich_dict[enrich_module]['info']['submodule'], [hit])

                hits = len(enrich_dict[enrich_module]['result']['hits']['hits'])
                moduleDidRun(enrich_module, 'enrich', 'success', 'Enriched %s documents' % hits, hits)
            ...
            # pylint: disable=broad-except
                ...
                moduleDidRun(enrich_module, 'enrich', 'error', msg)
...
def run_alarms(alarm_dict):
    ...
    for alarm_module in alarm_dict:
        if shouldModuleRun(alarm_module, 'redelk_alarm'):
            try:
                ...
                moduleDidRun(alarm_module, 'alarm', 'success', 'Found %s documents to alarm' % hits, hits)
            ...
            # pylint: disable=broad-except
                ...
                logger.error(msg)
                logger.exception(error)
                moduleDidRun(alarm_module, 'alarm', 'error', msg)
    ...
    for alarm in alarm_dict:
        if alarm in alarms and alarms[alarm]['enabled']:
            ...
            for result_hits in result['hits']['hits']:
                ...
                result_hits = addAlarmData(result_hits, mutations, alarm_name)

            # Let's tag the docs with the alarm name
            setTags(alarm_name, result['hits']['hits'])
            ...
            if result['hits']['total'] > 0:
                ...
                result['hits']['hits'] = groupHits(result['hits']['hits'], group_by)
