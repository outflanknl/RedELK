...
class IBMXforce:
    ...
    def checkIBMxForce(self):
        ...
        try:
            url = "https://exchange.xforce.ibmcloud.com/api/url/{}".format(self.domain)
            headers = {
                "User-Agent": useragent,
                "Accept": "application/json, text/plain, */*",
                "Accept-Language": "en-GB,en;q=0.5",
                "x-ui": "XFE",
                "Referer": "https://exchange.xforce.ibmcloud.com/url/{}".format(
                    self.domain
                ),
                "Connection": "close",
            }
            ...
            parsedJson = "{}".format(
                " | ".join(list(responseJson["result"].get("cats", {}).keys()))
            )
    ...
    def submit_category(self):
        ...
        post_data = (
            '{"feedback":{"sourceid":"%s","feedbacktext":"","current":{"urlcategory":[]},"proposed":{"urlcategory":[{"name":"Banking","action":"ADD","id":"53"}]},"webApplication":"","notify":true,"postAsComment":true}}'
            % self.domain
        )
