...
class Chameleon:
    ...
    def validate_args(self):
        ...
        parser.add_argument(
            "--check", action="store_true", help="Perform check on current category"
        )
        ...
        parser.add_argument(
            "--redelk", action="store_true", help="Enable RedELK integration"
        )
    ...
    def run(self, args):
        if args.redelk:
            fileout = open("/var/log/redelk/redteamdomaincheck.txt", "a+")
            print(
                "\033[1;34m[-] Checking local RedELK file /etc/redelk/roguedomains.conf\033[0;0m"
            )
            fileroguedomains = open("/etc/redelk/roguedomains.conf", "r")
            ...
            for line in fileroguedomains:
                if re.search(args.domain, line):
                    words = re.split("#", line)
                    print(
                        "\033[1;31m[-] Known malware according to RedELK roguedomains.conf\033[0;0m"
                    )
                    fileout.write(
                        datetime.now(timezone.utc).strftime("%Y/%m/%d, %H:%M:%S")
                        + " Domain: "
                        + args.domain
                        + " Source: RedELK wizardry Results: malware according to "
                        + words[1].strip()
                        + "\n"
                    )
            ...
            if args.check:
                ...
                if args.redelk:
                    fileout.write(
                        ...
                        + " Domain: "
                        + args.domain
                        + " Source: McAfee Trustedsource Results: "
                        + checkresults
                        + "\n"
                    )
            elif args.submit:
            ...
            if args.check:
                ...
                if args.redelk:
                    fileout.write(
                        ...
                        + " Domain: "
                        + args.domain
                        + " Source: Bluecoat WebPulse Results: "
                        + checkresults
                        + "\n"
                    )
            elif args.submit:
                ...
                while True:
                    ...
                    if choice == "Y" or choice == "y":
                        b = bluecoat.Bluecoat(
                            args.domain, "https://www.bankofamerica.com"
                        )
                        b.run()
                        break
